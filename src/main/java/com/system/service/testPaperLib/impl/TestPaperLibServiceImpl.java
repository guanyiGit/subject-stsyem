...
public class TestPaperLibServiceImpl implements TestPaperLibService {
    ...
    //查询题库每种类型或难度的数量
    @Override
    public Map<String, List<TestPapperClassification>> selectDifficultyOrTypeNum(Integer Type) throws ServerException{
        ...
        try {
            ...
            list2=tTestPaperMapper.selectDifficultyNum(Type);
            map.put("type",list1);
        ...
        try {
            ...
            if (difficultyList!=null && difficultyList.size()>0){
                for (int i=0;i<difficultyList.size();i++){
                    List<TSubjectRefPaper> listpaper2 = tTestPaperMapper.selectSubjectIdsByDifficulty(difficultyList.get(i).getType(),difficultyList.get(i).getSetNum(),parmQuestion.getPaperType());
    ...
    public void insertTestPaper(ParmQuestion parmQuestion) throws ServerException {
        ...
        try {
            ...
            if (parmQuestion.getPaperId()!=null){
                ...
                tTestPaperMapper.updateSubjectBank(list);
            }
        } catch (Exception e) {
            String log = "创建试卷失败！";
            throw new ServerException(log, e);
    ...
    @Transactional
    @Override
    public void copyTestPaper(Integer usrId,Integer paperId,String paperName) throws ServerException {
        List<TSubjectRefPaper> list = new ArrayList<TSubjectRefPaper>();
        TTestPaper tTestPaper = null;
        try {
            tTestPaper=tTestPaperMapper.selectByPrimaryKey(paperId);
            tTestPaper.setPaperId(null);
            tTestPaper.setPaperCreater(usrId);
            tTestPaper.setPaperName(paperName);
